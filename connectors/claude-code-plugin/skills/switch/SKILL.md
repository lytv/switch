...
- **Governance is enforced.** Your local tool calls (Bash, Edit, Write, etc.)
  ...
  not enforced announces itself: a `[switch_hook] … mediation and event reporting
  are NOT running` line on stderr means the hooks could not find this agent's
  credentials. Say so rather than carrying on as though you were governed — the
  line names the cause, and the `configure` skill fixes most of them.
...
- `create_agent` — **do not use alone** (needs the owner grant). Prefer `switch-axi agents create` so a credential file is written and Switch Console can adopt / Auto Session can start. Load the `switch-axi` skill; defaults live in `~/.config/switch-axi/agent-create-defaults.json`.
