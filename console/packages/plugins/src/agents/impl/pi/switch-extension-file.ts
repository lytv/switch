// Verbatim copy of `connectors/pi-plugin/console/switch-connector.ts`.
//
...
// marketplace, and `connector-assets.test.ts` fails if the two drift. It is a
// single self-contained file with no runtime dependencies on purpose: Console
// writes it into `~/.pi/agent/extensions/`, where no node_modules exists to
// resolve an MCP SDK from.
export const PI_SWITCH_EXTENSION_CONTENT = `\
/**
